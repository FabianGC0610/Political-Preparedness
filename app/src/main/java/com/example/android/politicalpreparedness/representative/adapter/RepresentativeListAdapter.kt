...
class RepresentativeViewHolder private constructor(private val binding: RepresentativeItemBinding) :
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) { enableLink(binding.facebookImageLink, facebookUrl) }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) { enableLink(binding.twitterImageLink, twitterUrl) }
    }

    private fun showWWWLinks(urls: List<String>) {
        enableLink(binding.wwwImageLink, urls.first())
    }
